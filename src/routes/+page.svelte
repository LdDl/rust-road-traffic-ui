<script lang="ts">
	...
	import {
		...
		updateDataStorage,
		clearDataStorage,
		resetZoneSpatialInfo,
		markZonesSaved,
		forgetSavedZones
	} from '../store/data_storage';
	...
	import {
		...
		drawCanvasPolygons,
		CustomPolygon,
		updateCanvasMeasurements
	} from '$lib/custom_canvas';
	...
	$: canvasFocused =
		stateVariable === States.AddingZoneCanvas || stateVariable === States.DeletingZoneCanvas;
	$: mapFocused =
		stateVariable === States.AddingZoneMap || stateVariable === States.DeletingZoneMap;
	...
	onMount(() => {
		...
		DeleteClickedZone.onClick = (s: any, e: any) => {
			if (e.featureTarget && stateVariable === States.DeletingZoneMap) {
				...
				let mustUpdateSpatial = [...$dataStorage].find(
					(f) => f[1].properties.spatial_object_id === spatialID
				)?.[1];
				if (!mustUpdateSpatial) {
					console.warn(
						`Spatial ID '${spatialID}' not found in datastorage. Just removing from the spatial map...`
					);
		...
		$map.on('draw.update', function (e: DrawUpdateEvent) {
			...
			if ($canvasState) {
				const canvasPolygon = $canvasState
					.getObjects()
					.find((obj) => obj instanceof CustomPolygon && obj.unid === mustUpdateSpatial!.id) as
					| CustomPolygon
					| undefined;
				if (canvasPolygon) {
...
<div id="main-app">
	...
	<div class="tab-panel" class:tab-hidden={$activeTab !== 'setup'}>
		...
		<div class="mobile-tab-bar">
			<button
				class="mobile-tab"
				class:active={mobileTab === 'view'}
				on:click={() => (mobileTab = 'view')}
			>
			...
			<button
				class="mobile-tab"
				class:active={mobileTab === 'zones'}
				on:click={() => (mobileTab = 'zones')}
			>
		...
		<div
			...
			style={isMobile
				? ''
				: `grid-template-columns: ${leftPanelWidth}% 2px ${100 - leftPanelWidth}%`}
		>
			<div
				id="left_workspace"
				bind:clientHeight={leftWorkspaceHeight}
				style={isMobile
					? ''
					: `grid-template-rows: ${topPanelHeight}% 2px ${100 - topPanelHeight}%`}
			>
				<div class="canvas-panel" class:mobile-hidden={mobileTab === 'zones'}>
					<CanvasComponent
						active={$activeTab === 'setup'}
						klass={!canvasFocused && mapFocused ? 'blurred noselect' : ''}
					/>
				</div>
				<div
				...
				<div class="zones-panel" class:mobile-hidden={mobileTab === 'view'}>
					<ConfigurationStorage
						{dataReady}
						data={dataStorageAll}
						klass={!$canvasReady || canvasFocused || mapFocused ? 'blurred noselect' : ''}
					/>
				</div>
				<div
					class="overlay"
					style={!canvasFocused && mapFocused ? 'display: flex;' : 'display: none;'}
				>
					<span
						>Press ESC to cancel '{cancelActionText !== undefined
							? cancelActionText
							: cancelActionUnexpected}' mode</span
					>
